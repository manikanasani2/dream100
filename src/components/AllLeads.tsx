    ...
    <div className="flex-1 bg-[#0B0B0F]">
      ...
      <div className="p-6 border-b border-[#2a2a2a]">
        <div className="relative">
          <Search className="absolute left-4 top-1/2 transform -translate-y-1/2 h-4 w-4 text-[#9CA3AF] transition-all duration-300" />
          <input
            type="text"
            placeholder="Search leads and companies..."
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
            className="futuristic-input w-full pl-12 pr-4 py-3 rounded-lg text-white placeholder-[#9CA3AF] focus:outline-none"
          />
        </div>
      </div>
        ...
        <table className="w-full">
          ...
          <tbody className="divide-y divide-[#2a2a2a]">
            {filteredLeads.map((lead) => (
              <tr 
                key={lead.process_id} 
                className="hover:bg-gradient-to-r hover:from-[#1a1a1a] hover:to-[#1f1f1f] transition-all duration-300 ease-out cursor-pointer group hover:shadow-lg hover:shadow-black/20"
                onClick={() => {
                  setSelectedLead(lead);
                  setShowDetailModal(true);
                }}
              >
                <td className="p-4">
                  <div className="text-white font-medium group-hover:text-accent-red transition-colors duration-300">{lead.lead_name}</div>
                </td>
                <td className="p-4">
                  <div className="text-[#9CA3AF] group-hover:text-white transition-colors duration-300">{lead.lead_company_name || '-'}</div>
                </td>
                <td className="p-4">
                  <div className="text-[#9CA3AF] group-hover:text-white transition-colors duration-300">{lead.industry || '-'}</div>
                </td>
                <td className="p-4">
                  <div className="text-[#9CA3AF] group-hover:text-white transition-colors duration-300">{lead.job_title || '-'}</div>
                </td>
                <td className="p-4">
